...
func (p *PromCache) Reset() {
	p.Close()
	p.init()
}

func (p *PromCache) init() error {
	var err error
...
func (p *PromCache) Load(series promql.Series) (err error) {
	refid := uint64(0)
...
func (p *PromCache) InstantQuery(q string, t time.Time) (promql.Value, error) {
	if p.needsCommit {
		p.needsCommit = false
		p.appender.Commit()
	}
...
// RangeQuery .
func (p *PromCache) RangeQuery(q string, start, end time.Time, interval time.Duration) (promql.Value, error) {
	if p.needsCommit {
		p.needsCommit = false
		p.appender.Commit()
	}
